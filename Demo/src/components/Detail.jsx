...
export default function Details() {
    ...
    const handleSubmit = async () => {
        try {
            let res;
            const userId = newUser.id.toString(); // Đảm bảo ID là chuỗi
            ...
            if (userId) {
                ...
                // Gửi yêu cầu POST nếu ID không tồn tại
            ...
            setUserData(prevData =>
                prevData.map(user => user.id.toString() === savedUser.id.toString() ? savedUser : user)
            );
            ...
            {isModalOpen && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-30 z-50">
                    <div className="bg-white p-6 rounded-lg w-[400px] space-y-4">
                        <h3 className="text-xl font-semibold">{newUser.name ? 'Edit User' : 'Add New User'}</h3>
                        <input
                            name="name"
                            value={newUser.name}
                        ...
                        <div className="flex justify-end gap-2">
                            <button onClick={() => setIsModalOpen(false)} className="px-4 py-1 border rounded">
                                Cancel
                            </button>
                            <button
